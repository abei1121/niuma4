...
import { DEFAULT_TRACK_ITEMS, STATIC_NODE_TYPES } from './constants';
import { buildWorkflowEdges } from './workflowEdges';
import { useStudioNodes } from './useStudioNodes';
import { useProjectManager } from './useProjectManager';
import { createTrackItemsFromSegments } from './studioCanvasActions';
...
const StudioCanvasComponent: FunctionalComponent = () => {
  ...
  }, [files.length, isL1ToL2Connected, keepSegments.length, cleanFile, washStats, isL2ToL3Connected, draftReady, trackItems.length, isL3ToL4Connected]);

  const onProjectLoaded = useCallback((_loadedName: string) => {
    if (rfInstance) {
      setTimeout(() => rfInstance.fitView?.({ padding: 0.12, duration: 400, maxZoom: 0.85 }), 100);
    }
  }, [rfInstance]);
  ...
  const { getCurrentProjectData, handleLoadProjectData, handleRotateFile } = useProjectManager({
    ...
    sectorId, setSectorId, subOptionId, setSubOptionId, directorPrompt, setDirectorPrompt, topic, setTopic,
    draftReady, setDraftReady, setIsL1ToL2Connected, setIsL2ToL3Connected, onProjectLoaded,
  });
  ...
  const handleApplyCutToL3 = useCallback((segments: KeepSegmentItem[]) => {
    ...
    const newTracks = createTrackItemsFromSegments(segments);
  ...
  const handleNewProject = useCallback(() => {
    ...
    setCutSegments([]);
    setSectorId('xuanxue');
    setSubOptionId('xx_mingpan');
    setDirectorPrompt('爆款自媒体玄学名人命盘视频导演');
    setTopic('');
    ...
    setTrackItems(DEFAULT_TRACK_ITEMS);
    resetNodesLayout();
    if (rfInstance) setTimeout(() => rfInstance.fitView?.({ padding: 0.12, duration: 400, maxZoom: 0.85 }), 50);
  }, [rfInstance, resetNodesLayout]);
